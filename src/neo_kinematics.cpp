...
void NeoKinematics::ForwardVelKinematics(std::vector<double> vdDriveGearVelRadS,std::vector<double> vdSteerGearVelRadS,std::vector<double> vdDriveGearDltAngRad,std::vector<double> vdSteerGearAngRad)
{
	...
	for(int i = 0; i<m_iNumOfJoints; i++)
	{
	...
	for(int i = 0; i< (m_iNumOfJoints-1) ; i++)
	{
	...
	// transform velocity of wheels into relative coordinate frame of linking axes -> subtract angles
	dtempRelPhiWheel1RAD = m_vdSteerGearAngRad[m_iNumOfJoints-1] - dtempRelPhiWheelsRAD;
	dtempRelPhiWheel2RAD = m_vdSteerGearAngRad[0] - dtempRelPhiWheelsRAD;

	// close calculation of robots rotational velocity
	dtempRotRobRADPS += (vdtempVelWheelMMS[0]*sin(dtempRelPhiWheel2RAD) - vdtempVelWheelMMS[m_iNumOfJoints-1]*sin(dtempRelPhiWheel1RAD))/dtempRelDistWheelsMM;
	...
	m_dVel_x_cmd = dtempVelXRobMMS/m_iNumOfJoints;
	m_dVel_y_cmd = dtempVelYRobMMS/m_iNumOfJoints;

}

void NeoKinematics::InverseVelKinematics(double dVel_x_cmd,  double dVel_y_cmd, double dVel_rad_cmd)
{
...
void NeoKinematics::GetRefreshedCtrlState(	std::vector<double> m_vdDriveGearVelRadS, std::vector<double> m_vdSteerGearVelRadS, std::vector<double> m_vdSteerGearAngRad, double m_dVel_x_cmd, double m_dVel_y_cmd, double m_dVel_rad_cmd)
{
	// if condition ? 
